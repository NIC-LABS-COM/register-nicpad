sap.ui.define([
	...
	return BaseController.extend("sap.ui.demo.todo.controller.PersonList", {
		...
		onInit: function () {
			...
			this.getRouter().getRoute("personList").attachMatched(this._onRouteMatched, this);
		...
		onEditPerson: function (oEvent) {
			...
			if (selectedItems.length === 0) {
				MessageToast.show(this.getResourceBundle().getText("msgSelectARecord"));
			} else if (selectedItems.length > 1) {
				MessageToast.show(this.getResourceBundle().getText("msgSelectOnlyOneRecord"));
			} else {
		...
		onDeletePerson: function (oEvent) {
			...
			if (selectedItems.length > 0) {
				MessageBox.confirm(
					this.getResourceBundle().getText("msgQuestionDeletePerson"), {
					onClose: function (oAction) {
						if (oAction === sap.m.MessageBox.Action.OK) {
							...
							if (iSuccess == selectedItems.length) {
								MessageToast.show(that.getResourceBundle().getText("msgRecordDeleted"));
							} else {
								if (notAuthorized) {
									that.getRouter().navTo("login");
								} else {
									MessageToast.show(that.getResourceBundle().getText("msgRecordDeleteError"));
								}
							}
							that._getPersonList();
						}
				...
				MessageToast.show(this.getResourceBundle().getText("msgSelectAtLeastARecord"));
			}
		},
		...
		_getPersonList: function (sSearchQuery) {
			var url = "/person";
			if (sSearchQuery) {
				url = url + "?" + "cpf: { $regex: " + "'" + sSearchQuery + "'" + " }";
			}
			var that = this;
